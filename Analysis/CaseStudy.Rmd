...
```{r}
load("/Users/yiqianzhang/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Collaboration With Zihan/PERMANOVA/Real Data/Mouse Gut/phy(15900).RData")
sample_data <- sample_data(phy)
rows_with_na <- apply(sample_data[, c(7, 35, 40, 24)], 1, function(x) any(x %in% c("na", "not applicable", "")))
...
metadata <- data.frame(
  # Variable of interest
  condition = as.factor(physeq@sam_data$condition),
  host_age = as.numeric(physeq@sam_data$host_age), 
  host_sex = as.factor(physeq@sam_data$host_sex),
...
# Display the results table
print(results_table)
write.csv(results_table, "CaseStudyResult/results_table_MouseGut.csv", row.names = TRUE)
```
...
# Concordance_oral_wgs
```{r}
load("~/Library/CloudStorage/Box-Box/WenhaoLi@MDA/BetaDiversity/real data/Concordance_oral/WGSphylo/WGSphylo_w_tree.Rdata")
sample_data <- sample_data(physeq)
rows_with_na <- apply(sample_data[, c(3, 2, 4, 5, 25, 27, 42, 43)], 1, function(x) any(x %in% c("missing", "not provided", "")))
physeq <- prune_samples(!rows_with_na, physeq)
...
# Display the results table
print(results_table)
write.csv(results_table, "CaseStudyResult/results_table_Concordance_oral_wgs.csv", row.names = TRUE)
```

# Tumer
```{r}
load("/Users/yiqianzhang/Desktop/GLaD/Analysis/RealData/tumor_rooted.RData")
sample_data <- sample_data(physeq.tree)

# Remove last character from group and assign back
sample_data$group <- substr(sample_data$group, 1, nchar(as.character(sample_data$group)) - 1)

# Filter physeq.tree to keep only samples in LTS or STS (after trimming)
physeq.tree@sam_data <- sample_data
physeq_sub <- subset_samples(physeq.tree, group %in% c("LTS", "STS"))
...
physeq <- prune_taxa(keep_otus, physeq_sub)


# Prepare metadata
metadata <- data.frame(
  # Variable of interest bmi, sex, educ, host_age,raceethn, host_height,host_weight,adipnectin
  group = physeq@sam_data$group,
  Cohort = physeq@sam_data$Cohort
)
...
# Display the results table
print(results_table)
write.csv(results_table, "CaseStudyResult/results_table_tumor.csv", row.names = TRUE)
```
